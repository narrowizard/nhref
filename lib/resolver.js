 ...
 * @param callback 成功回调
 */
function resolveScripts(scripts, styles, filepath, callback) {
    var pending = 0;
    var count = scripts.length + styles.length;
    // script
...
function resolveSrc(src, filepath, callback) {
    ...
    if (h.test(src)) {
        ...
        // 协议跟随的情况 //开头的路由
        request("http:" + src, function (err, response, body) {
            // 首先尝试http
            if (response.statusCode == 404) {
                // 使用https
                request("https:" + src, function (err, response, body) {
                    if (!err && response.statusCode == 200) {
                        var md5 = crypto.createHash('md5').update(body).digest('hex');
                        callback(md5);
                    } else {
        ...
        var target = path.join(global.rootPath, urlObject.pathname);
